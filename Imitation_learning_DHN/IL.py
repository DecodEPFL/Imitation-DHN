...
u = torch.tensor([0, 8.7007, 8.9493, 9.2064, 9.5232, 9.8074, 9.9837,
...
x_u_train = torch.cat((x_train, u_train), dim=-1)
...
for epoch in range(epochs):
    ...
    for kk in range(n_traj):
        ...
        for t in range(t_end):
            ...
            usys = u_traj[t]  # Control input at time t

            # Compute an estimate of the disturbance
            x_ = x_traj[t - 1] if t > 0 else x0  # Use previous state or x0
            w_REN = x - sys.noiseless_forward(t, x_, usys)
            u_pred, xi = ctl(t, w_REN, xi)  # Predict control input
            #delta = classifier(w_REN)


            # Compute the loss as the MSE between predicted and actual input
            MSE_loss += MSE_loss_fn(u_pred, usys) / n_traj
...
u_test_log = torch.zeros_like(u_test)  # Log for predicted inputs
...
for kk in range(n_traj):
    ...
    for t in range(t_end):
        x = x_traj[t]  # State at time t
        usys = u_traj[t]  # Actual input from data

        # Compute the REN residual
        x_ = x_traj[t - 1] if t > 0 else x0  # Use previous state or x0
        w_REN = x - sys.noiseless_forward(t, x_, usys)  # Calculate disturbance
        u_pred, xi = ctl(t, w_REN, xi)  # Predict control input

        # Log the predictions and disturbances
        x_test_log[kk, t] = x
        u_test_log[kk, t] = u_pred
        w_test_log[kk, t] = w_REN  # Log the disturbances

        # Compute the test loss as MSE between predicted and actual input
        test_MSE_loss += MSE_loss_fn(u_pred, usys) / n_traj
...
for kk in range(min(3, n_traj)):  # Plot up to 3 test trajectories
    plt.plot(u_test[kk, :, 0].numpy(), label=f'Test u_actual_{kk}', linestyle='--')
    plt.plot(u_test_log[kk, :, 0].detach().numpy(), label=f'Test u_pred_{kk}')
