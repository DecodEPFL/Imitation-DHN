...
state_data = loadmat('X.mat')['X_all']
input_data = loadmat('U.mat')['U_all']
x_dataset = torch.zeros(n_traj, t_end, sys.n)
u_dataset = torch.zeros(n_traj, t_end, sys.n)
x_u_dataset = torch.cat((x_dataset, u_dataset), dim=-1)
llabels = (u_dataset > 0.1).float()


# Populate the dataset
for i in range(n_traj):
    x_dataset[i, :, :] = torch.tensor(state_data[:, i]).float().unsqueeze(-1)
    u_dataset[i, :, :] = torch.tensor(input_data[:, i]).float().unsqueeze(-1)
...
for epoch in range(epochs):
    ...
    for kk in range(train_size):
        ...
        x000 = 30*torch.ones(1, x_traj.shape[1])
        # Concatenate the zero tensor to the beginning of x_traj along the time dimension (dim=0)
        x_traj_with_x0 = torch.cat((x000, x_traj), dim=0)
        u_traj = u_train[kk]  # Input trajectory: (t_end, sys.m)
        xi = torch.zeros(ctl.n)  # Reset REN internal state

        for t in range(t_end):
            x = x_traj_with_x0[t]  # State at time t
            u = u_traj[t]  # Control input at time t
            x_prev = x_traj_with_x0[t-1] if t > 0 else 0
...
test_MSE_loss = 0
x_test_log = torch.zeros_like(x_train)  # Log for predicted states
u_test_log = torch.zeros_like(u_train)  # Log for inputs
...
test_MSE_loss = test_MSE_loss / t_end
delta_pred_test = classifier(x_u_test)

test_class_loss = class_loss_fn(delta_pred_test, labels_test)/ t_end
...
for kk in range(min(3, n_traj-train_size)):  # Plot up to 3 test trajectories
    plt.subplot(2, 1, 1)
...
for kk in range(min(3, n_traj)):  # Plot up to 3 test trajectories
    ...
    predicted_labels = (delta_pred_test[kk] > 0.5).float().detach().numpy()
